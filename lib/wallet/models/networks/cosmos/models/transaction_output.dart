...
class CosmosOutputWithBalance {
  CosmosOutputWithBalance(
      ...
      : _balance = IntegerBalance.zero(network.token, allowNegative: false);
  ...
  void setToken(CW20Token token, WalletCosmosNetwork network) {
    if (token.denom == network.coinParam.denom) {
      _token = null;
      _balance = IntegerBalance.token(BigInt.zero, network.token);
    } else {
      _token = token;
      _balance = IntegerBalance.token(BigInt.zero, _token!.token);
    }
    _checkValue();
  }
  ...
  CosmosWalletTransactionTransferOutput toOutput(WalletCosmosNetwork network) {
...
class CosmosTransactionFeeInfo {
  ...
  factory CosmosTransactionFeeInfo.customFee(
    ...
    return CosmosTransactionFeeInfo._(
        ...
        feeAmount: IntegerBalance.token(fee ?? BigInt.zero, token.token),
        maxFee: token.balance.balance,
        gasLimit: gasLimit,
        feeDenom: token.denom,
        networkFeeRate:
            IntegerBalance.token(networkFeeRate ?? BigInt.zero, token.token),
