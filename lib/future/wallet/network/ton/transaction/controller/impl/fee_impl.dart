...
mixin TonFeeImpl {
  ...
  late TonTransactionFeeDetails _feeDetails =
      TonTransactionFeeDetails.nonEstimate(network);
  ...
  Future<void> estimateFee() async {
    ...
    _feeDetails = TonTransactionFeeDetails.nonEstimate(network);
    onChange();
    ...
    if (estimate.hasError) {
      if (!estimate.isCancel) {
        ...
        if (estimate.errorISA<WalletException>()) {
          _feeError = estimate.error;
        } else {
