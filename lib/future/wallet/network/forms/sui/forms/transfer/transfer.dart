...
class SuiTransferForm extends SuiTransactionForm {
  ...
  BigInt max(SuiOutputWithBalance receiver, BigInt fee) {
    if (transactionType.isTokenTransfer) {
      _transferValue =
          destination.value.fold(BigInt.zero, (p, c) => p + c.balance.balance);
      final max =
          (token!.balance.balance - _transferValue + receiver.balance.balance);
      return max;
    }
  ...
  Future<SuiProgrammableTransaction> _createTokenTransaction() async {
  ...
  SuiWalletTransaction toWalletTransaction({required String txId}) {
    ...
    return SuiWalletTransaction(
        ...
        totalOutput: WalletTransactionIntegerAmount(
            amount: total, network: network, token: token?.token),
        network: network);
  }
}
