...
class SubstrateTransferForm extends SubstrateTransactionForm {
  ...
  SubstrateWalletTransaction toWalletTransaction({
    ...
    final destinations = destination.value;
    final outputs =
        destinations.map((e) => e.toWalletTransactionOutput(network)).toList();
    return SubstrateWalletTransaction(
        ...
        totalOutput: WalletTransactionIntegerAmount(
            amount: destinations.fold<BigInt>(
                BigInt.zero, (p, c) => p + c.balance.balance),
            network: network),
