...
class SolanaCreateAccountForm extends SolanaTransactionForm {
  ...
  Future<void> _getLamportsForRent() async {
    ...
    final rent = await MethodUtils.call(() async {
      return await client.getRent(space.value!.toBigInt().toInt());
    });
    if (rent.isCancel) return;
    if (!_manuallyLamports) {
      setValue(lamports, IntegerBalance.token(rent.result, network.token));
  ...
  void setSpace<T>(BigRational? val) {
    ...
    setValue(lamports, IntegerBalance.token(BigInt.zero, network.token));
    setValue(space, val);
    _getLamportsForRent();
  }
  ...
  void setLamports(BigInt? val) {
    ...
    setValue(lamports, IntegerBalance.token(val, network.token));
