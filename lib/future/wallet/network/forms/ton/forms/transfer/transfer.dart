...
class TonTransferForm extends TonTransactionForm {
  ...
  BigInt getActiveTokenBalance(TonOutputWithBalance output) {
    ...
    return output.jetton!.balance.balance - spendedValue;
  }
  ...
  @override
  List<MessageRelaxed> toMessages() {
    return destination.value
        .map((e) => e.toMessage(network, address.networkAddress))
        .toList();
  }
  ...
  TonWalletTransaction toWalletTransaction({required String txId}) {
    final outputs = destination.value
        ...
        .toList();
    final total = destination.value
        .fold<BigInt>(BigInt.zero, (p, c) => p + c.balance.balance);
