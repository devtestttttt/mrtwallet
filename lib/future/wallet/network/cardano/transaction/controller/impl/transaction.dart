...
abstract class CardanoTransactionImpl extends WalletSendTransactionStateImpl<
    ICardanoAddress, CardanoClient, WalletCardanoNetwork, ADAChain> {
  ...
  List<ICardanoAddress> get addresses => account.addresses;
  ...
  Future<void> fetchUtxos() async {
    ...
    for (final i in _addresses) {
      ...
      try {
        final utxos = await apiProvider.getUtxos(address: i, chain: account);
        _fetchedUtxos[i.networkAddress.address] =
  ...
  bool _isReady() {
    ...
    final bool assetIsReady = (!hasAsset ||
        (_changeAssetOutput.isReady ||
            (!_changeAssetOutput.hasAssets && !_changeAssetOutput.hasAmount)));
  ...
  Future<void> onAddRecever({
    ...
    final addresses = await onSelectAddresses(
        account: account, onFilterAccount: validateAddress);
