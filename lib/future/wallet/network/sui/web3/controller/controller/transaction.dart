...
import 'package:on_chain_wallet/future/wallet/network/forms/sui/sui.dart';
...
class Web3SuiTransactionRequestController
  ...
  Future<void> _saveTransaction({required String txId}) async {
    final s = await MethodUtils.call(() async {
      final SuiWalletTransaction transaction = SuiWalletTransaction(
          txId: txId,
          outputs: [],
    ...
    if (isExecute) {
      final execute = await MethodUtils.call(() async {
        ...
        await _saveTransaction(
            txId: txId.digest ?? signedTransaction.result.digest);
        return txId;
      }, cancelable: _cancelable);
      if (execute.hasError) {
      ...
      final response = Web3SuiSignAndExecuteTransactionResponse(
          digest: execute.result.digest ?? signedTransaction.result.digest,
