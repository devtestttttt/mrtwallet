...
import 'package:on_chain_wallet/future/wallet/network/bitcoin/web3/controller/controller/transaction.dart';
  ...
  Widget build(BuildContext context) {
    return SliverToBoxAdapter(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        InsufficientBalanceErrorView(balance: form.insufficientBalances),
        ErrorTextContainer(error: form.error, enableTap: false),
        Row(
...
class __SelectUtxosState extends State<_SelectUtxos>
  ...
  void updateAmount() {
    ...
    for (final i in selectedutxos) {
      final token = i.cashToken?.balance.token;
  ...
  void _init() {
    ...
    totalSelect[network.token] = IntegerBalance.zero(network.token);
    final allUtxos =
        utxos.map((e) => e.utxosWithBalance).expand((e) => e).toList();
    for (final i in allUtxos) {
      final token = i.cashToken?.balance.token;
      if (token == null) continue;
      totalSelect[token] ??= IntegerBalance.zero(token);
    }
    updateAmount();
  }
        ...
        slivers: [
          ...
          SliverConstraintsBoxView(
            ...
            sliver: EmptyItemSliverWidgetView(
              ...
              itemBuilder: (context) => MultiSliver(children: [
